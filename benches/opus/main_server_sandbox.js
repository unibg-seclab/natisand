...
const dylib = Deno.dlopen(
  libName,
  {
    ...
    "ope_encoder_create_file": {
      parameters: ["buffer", "pointer", "usize", "usize", "usize", "pointer"],
      result: "pointer",
    },
    "ope_encoder_write": {
...
function def_comments() {
  ...
  dylib.symbols.ope_comments_add(
    comments_ptr,
    toCString("ARTIST"),
    toCString("Someone"),
  );
  dylib.symbols.ope_comments_add(
    comments_ptr,
    toCString("TITLE"),
    toCString("Some track"),
  );
  return comments_ptr;
}

function encode(bytes, size, enc) {
  var ct = 0;
  while (ct < size) {
    let b_size;
    if (size - ct < 1024) {
...
const handler = async (request) => {
  ...
  let comments_ptr = Deno.core.ops.nativeCall(def_comments, undefined, []);

  let enc = dylib.symbols.ope_encoder_create_file(
    toCString(outputFilePath),
    comments_ptr,
    48000,
    2,
    0,
  ...
  Deno.core.ops.nativeCall(encode, undefined, [bytes, size, enc]);
  Deno.core.ops.nativeCall(clean, undefined, [enc, comments_ptr]);
